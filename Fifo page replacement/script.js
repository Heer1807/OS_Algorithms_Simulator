// Initializing these variables outside all the functions so that they can be 
// used by all the functions 
let pageFaults, pageHits, pageReferences; 

function fifoPageReplacement(pages, capacity) { 
    // Initializing the values of page faults, the pageFrame queue and a 
    // dynamic output which can be altered according to the needs. 
    pageFaults = 0; 
    let pageFrame = []; 
    let output1 = ""; 
    ...
    for (let i = 0; i < pages.length; i++) { 
        ...
        if (pageIndex === -1) { 
            ...
            if (pageFrame.length < capacity) { 
                ...
                // Print that the element has been loaded into the frame by incrementing the value of output 
            ...
            // if the pageFrame queue is full 
                ...
                // Enter the new page 
                pageFrame.push(pages[i]); 
                // Increment the output and print that the oldest element in the queue has been replaced 
    ...
    output1 += `<p>The total references are: ${pages.length}</p>`; 
...
function runSimulation() { 
    ...
    let numFrames = parseInt(document.getElementById("num-frames").value); 
    pageReferences = document.getElementById("page-references").value.split(","); 

    if (isNaN(numFrames) || numFrames <= 0) { 
        alert("Number of frames must be a positive integer greater than 0"); 
        return; 
    } 

    // Parse all page references to integers and validate 
    for (var i = 0; i < pageReferences.length; i++) { 
        pageReferences[i] = parseInt(pageReferences[i].trim()); 
        if (isNaN(pageReferences[i])) {
            alert("Please enter a valid list of page references separated by commas."); 
            return;
        }
    } 
    ...
    document.getElementById("output0").innerHTML = "The reference array entered by the user is: " + pageReferences.join(", "); 
